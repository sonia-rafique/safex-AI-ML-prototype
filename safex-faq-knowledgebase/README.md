...
The FAQ Knowledge Base serves as the primary informational core of the SafeX Solutions WhatsApp Auto-Reply Bot. The purpose of this module is to establish a pipeline that bridges unstructured corporate web assets with a structured, query-responsive automated conversational endpoint.
...
The scraper operates as a web crawler starting from the homepage.
State Management: Uses a First-In-First-Out (FIFO) queue for Breadth-First Search link traversal up to 40 pages.
HTTP Adapters: Uses a custom HTTP session configured with urllib3.util.retry.Retry to handle network fluctuations, rate limits (429), and server-side errors (5xx).
Data Cleansing: Decomposes non-content HTML structures such as <script>, <style>, and <noscript> blocks using BeautifulSoup's .decompose() method. It avoids compressing payloads to prevent decoding issues.

B. Knowledge Base Processor (src/data_processor.py)
The data processor normalizes the raw data and prepares it for query matching.
Dual-Data Architecture: Loads 13 highly authoritative curated base records ensuring high-reliability service, pricing, and contact replies.
...
Lifespan Manager: Loads the compiled JSON records into memory at server startup to enable fast in-memory query processing.
...
Confidence Threshold Gate: Disallows queries that do not score at or above a minimum score of 20, keeping off-topic inquiries from returning misleading answers.
...
The project repository is organized under the following standard structure:
...
Run the pipeline stages from your project directory:

# Execute deep crawling
python -m src.scraper
# Compile hardcoded and AI-generated records
...
Out-Of-Domain Fallback Sample Input
Plaintext
